...
def feature_engineering(df):
    ...
    if 'start_datetime' in df.columns:
        ...
        # We want to compute rolling historical risk rate for each zone
        # To prevent leakage, for row i, we only look at rows < i
        # rolling target: 'priority_numeric' or 'priority_target'
        # Since 'priority' is high/low, target is 1 for high, 0 for low.
        df['priority_target'] = df['priority'].map({'high': 1, 'low': 0})
        
        # Expanding mean per zone shifted by 1 (to not include current row)
        df['zone_historical_risk'] = df.groupby('zone')['priority_target'].transform(
            lambda x: x.shift(1).expanding().mean()
        ).fillna(0.5) # Default to 0.5 if no history
    else:
        # Fallback if no start_datetime
