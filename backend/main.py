...
from services.v2_voice_normalizer import extract_location_from_text
import os
import shutil
import json
import uuid
        ...
        while True:
            ...
            if "bytes" in message and message["bytes"] is not None:
                ...
                try:
                    ...
                    transcript = transcribe_streaming_audio(temp_path)
                    normalized_transcript = normalize_streaming_transcript(transcript)
                    print("RAW STREAMING TRANSCRIPT:", transcript)
                    print("NORMALIZED STREAMING TRANSCRIPT:", normalized_transcript)
                    expected_field = get_v2_expected_field(current_incident)
                    ...
                    if expected_field == "severity":
                        severity = parse_v2_severity(normalized_transcript)
                        if severity:
                            ...
                            if severity_retry_count >= 2:
                                default_severity = get_v2_default_severity(current_incident)
                                current_incident["severity"] = default_severity
                    ...
                    if normalized_transcript and expected_field != "severity":
                        update_v2_current_incident(current_incident, normalized_transcript, expected_field)

                    complete, next_question = get_v2_next_question(current_incident)
                    print("V2 CURRENT INCIDENT:", current_incident)
